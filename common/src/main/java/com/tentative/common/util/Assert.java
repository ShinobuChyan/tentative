...
public class Assert {
    ...
    private final static String NOT_NULL = "参数列表不正确";

    private final static String TOO_LONG = "参数长度不正确";

    private final static String TOO_SHORT = "参数长度不正确";

    private final static String MATCH_FAILED = "参数格式有误";

    private final static String NOT_EMPTY = "参数不得为空";
    ...
    public static void length(Integer min, Integer max, String... strings) {
        ...
        for (int i = 0; i < strings.length; i++) {
            ...
            if (max != null && strings[i].length() > max) {
                throw new AssertFailedException(TOO_LONG);
            }
        }
    }
